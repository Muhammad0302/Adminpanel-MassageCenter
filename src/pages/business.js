...
import {
  ...
  editTextBusiness,
  deleteTextBusiness,
} from '../features/users/businessSlice';
...
export default function Masseuse() {
  ...
  const addAdminAgree = () => {
    ...
    if (value && imageUpload) {
      ...
      dispatch(addTextBusiness(formdata));
      setaddOpen(false);
      setValue();
      setImage();
    } else {
  ...
  const editAdminAgree = () => {
    ...
    if (editValue || imageUpload) {
      ...
      dispatch(editTextBusiness(editId, editdata));
      seteditOpen(false);
      setEditValue();
      setDefaultContent();
      setImage();
  ...
  const deleteAdminAgree = () => {
    ...
    dispatch(deleteTextBusiness(deleteId));
    setDeleteId();
  };
  ...
  const handleSelectAllClick = (event) => {
    if (event.target.checked) {
      const newSelecteds = business.map((n) => n.id);
  ...
  const filteredUsers = business?.filter((item) => {
    return filterName !== ''
  ...
  return (
    <Page title="User">
      ...
      >
        ...
        <DialogContent>
          <DialogContentText>Enter required Information in given fields to Add Business</DialogContentText>
          <ReactQuill
        ...
        <DialogActions>
          ...
          <Button onClick={addAdminAgree}>Add Business</Button>
        </DialogActions>
      </Dialog>
      ...
      <Dialog
        ...
        sx={{ maxWidth: '100vw', display: 'flex', justifyContent: 'center' }}
      >
        <DialogTitle sx={{ fontSize: '20px', display: 'flex', justifyContent: 'center' }}>Edit Business</DialogTitle>
        <DialogContent>
      ...
      <Dialog
        ...
        sx={{ maxWidth: '100vw', display: 'flex', justifyContent: 'center' }}
      >
        <DialogTitle sx={{ fontSize: '20px', display: 'flex', justifyContent: 'center' }}>Delete Business</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">Click Agree to delete business</DialogContentText>
        </DialogContent>
        <DialogActions>
          ...
          <TableContainer style={{ overflowX: 'auto' }}>
            <Table style={{ minWidth: '800' }}>
              ...
              <TableBody style={{ borderRight: '0.70432px solid #DEDDDD', borderLeft: '0.70432px solid #DEDDDD' }}>
                {filteredUsers?.slice(page * rowsPerPage, page * rowsPerPage + rowsPerPage)?.map((business) => {
                  const isItemSelected = selected.indexOf(business.id) !== -1;
                  ...
                  return (
                    ...
                    >
                      <TableCell padding="checkbox">
                        <Checkbox checked={isItemSelected} onChange={(event) => handleClick(event, business.id)} />
                      </TableCell>
